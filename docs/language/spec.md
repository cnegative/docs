...
- builtin stdlib modules for math, growable bytes/lines/text storage, strings, parsing, files, IO, low-level terminal control, environment, paths, time, blocking IPv4 TCP/UDP, process helpers, and an experimental Linux-only `std.x11` window path
